# Elo 3239L on Cage + Wayland

This guide is for an Elo 3239L (ET3239L-8CNA, PCAP) with the Elo 2700
IntelliTouch USB controller.

**Confirmed configuration on `.80` (production):**

- USB vendor/product: `04e7:0020`
- Kernel driver: `hid-generic` — presents the device as an absolute pointer (`/dev/input/event*`)
- Elo MT USB driver bundle installed in `/etc/opt/elo-mt-usb/` via `scripts/install-elo-driver.sh`
- `elo.service` enabled; runs `elomtusbd --stdigitizer` at boot to register a uinput device, then exits
- Touch input reaches Cage through the `hid-generic` absolute pointer path
- `uinput` module loaded at boot via `/etc/modules-load.d/uinput.conf`
- Compositor: `cage` / Wayland

The main issue being addressed is that the touchscreen is exposed as both a
touchscreen and a pointer-like device, which can make Cage/libinput show a
cursor after the first touch and can produce conflicting input behavior.
...
The stable target state (confirmed on `.80`):

- `elo.service` enabled; runs at boot and exits cleanly after `elomtusbd` initialises
- `uinput` module loaded (`/etc/modules-load.d/uinput.conf`)
- `hid-generic` kernel driver handles the Elo device as an absolute pointer
- `99-elotouch.rules` and `99-elo-usb-power.rules` present in `/etc/udev/rules.d/`
