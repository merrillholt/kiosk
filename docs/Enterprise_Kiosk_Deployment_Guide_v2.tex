...
Server hosted on one kiosk node:
\begin{lstlisting}
http://192.168.1.80/
http://192.168.1.80/admin
\end{lstlisting}

Other kiosks connect via LAN.
...
T10 & Companies CSV export & Download CSV & File downloads; header row present even with 0 rows \\
T11 & Companies CSV import & Upload valid CSV & Success status and expected row count in DB \\
T12 & Individuals CSV import & Upload valid CSV & Success status and expected row count in DB \\
T13 & Unauthorized CSV access & Request without auth & HTTP 401 Unauthorized \\
T14 & Data version change & Modify company/individual & \texttt{/api/data-version} increments \\
T15 & Deploy kiosk scripts & Admin deploy tab or script & Deploy reports success; target script updated \\
T16 & Power button long press & Hold power button on client & System powers off cleanly \\
\end{longtable}
...
R4 & Empty database export & CSV download still returns valid header row \\
...
\item Execute T10--T13 for any CSV/auth-related changes.
\item Execute T15 for any kiosk script changes.
...
\item Admin auth or CSV workflows fail.
