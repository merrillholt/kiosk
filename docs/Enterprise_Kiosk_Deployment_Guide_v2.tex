...
first and fall back to the standby if the primary is unreachable. If startup
selected standby, a conservative background watcher probes the primary on a long
fixed interval and only restarts the kiosk after sustained primary health.
...
\item \textbf{Deploy kiosk system scripts to display clients:}
  use Admin Deploy tab (\texttt{POST /api/kiosks/:id/deploy}) or \texttt{tools/deploy-ssh.sh --client}
\end{itemize}
...
\item Kiosk script deploy command: \texttt{tools/deploy-ssh.sh --client}
...
sudo systemctl restart directory-server
\end{lstlisting}
\item Deploy kiosk script updates from Admin Deploy tab (or \texttt{tools/deploy-ssh.sh --client})
